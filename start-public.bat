@echo off
chcp 65001 >nul
title 妙课生花 · 公网分享模式（本机跑 + 隧道穿透）
cd /d "%~dp0"
setlocal

echo ============================================================
echo   妙课生花 WonderKourse · 公网分享模式
echo   程序跑在你自己电脑上，通过隧道给公网一个网址
echo ============================================================
echo.
...
if not exist "tools\bin\cloudflared.exe" (
    echo   [!] 没找到穿透工具 tools\bin\cloudflared.exe
    echo       请先双击运行：tools\get-cloudflared.bat
    echo.
    pause
    exit /b 1
)
...
echo   第 3 步：建立隧道
echo ------------------------------------------------------------
echo   下面会打印一行 https://xxxx.trycloudflare.com —— 那就是分享网址。
