...
设了之后走**自带的登录页 + Cookie 会话**：

```
浏览器打开任何页面 -> 自动跳到 /login -> 输入口令 -> 进入站点
```

不再需要记用户名，也不用管浏览器会不会弹框。
...
| 行为 | 说明 |
|---|---|
| 登录页 | `/login`，整页自带样式，不依赖任何被保护的静态资源 |
| 退出 | 访问 `/logout` 即可清掉会话 |
| 会话时长 | 30 天；改口令会立即失效所有旧会话 |
| 跨重启 | Cookie 由口令派生，服务重启后仍然有效（不用反复登录） |
| 防暴力破解 | 同一 IP 连续错 10 次锁 10 分钟 |
| 免口令路径 | `/login`、`/logout`、`/api/health`、`/favicon.ico`、`/static/logo.svg` |
...
| 事项 | 说明 |
|---|---|
| **访问口令** | Public Space 意味着「网址公开」，但 `ACCESS_PASSWORD` 仍然会把内容挡在门外（会跳到自带登录页） |
